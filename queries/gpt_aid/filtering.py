...
# Load the dataset
df = pd.read_csv('gpt_queries_2_relevant_check_windfarms.csv')

# Filter by protocol and distance
siemens = df[(df['protocol'] == "siemens") & (df['distance'] < 100)]
modbus  = df[(df['protocol'] == "modbus") & (df['distance'] < 100)]
telnet  = df[(df['protocol'] == "telnet") & (df['distance'] < 100)]
ftp     = df[(df['protocol'] == "ftp") & (df['distance'] < 50)]
dnp3    = df[(df['protocol'] == "dnp3") & (df['distance'] < 200)]

# Combine filtered slices into one DataFrame
df_2 = pd.concat([siemens, modbus, telnet, ftp, dnp3]).drop_duplicates().reset_index(drop=True)
